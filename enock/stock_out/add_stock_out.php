<?php
$page_title = "Record Stock Out";
include '../includes/db_connection.php';

$message = "";
$message_type = "";

$parts_sql = "SELECT part_id, name, unit_price, quantity FROM spare_parts WHERE quantity > 0 ORDER BY name";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get current unit price from spare parts table
    $price_sql = "SELECT unit_price, quantity FROM spare_parts WHERE part_id = ?";
    ...
    if ($quantity > $current_quantity) {
        $message = "Error: Insufficient quantity available. Current stock: " . $current_quantity;
        $message_type = "error";
    } else {
        ...
        $sql = "INSERT INTO stock_out (part_id, quantity, stock_out_unit_price, stock_out_total_price, stock_out_date, user_id) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiddsi", $part_id, $quantity, $unit_price, $total_price, $stock_out_date, $_SESSION['user_id']);
        ...
        if ($stmt->execute()) {
            ...
            $message = "Error adding stock out record: " . $conn->error;
            $message_type = "error";
        }
        $stmt->close();
    }
...
<div class="form-container">
    ...
    <form method="POST" action="">
            ...
            <select id="part_id" name="part_id" required>
                ...
                <?php while ($part = $parts_result->fetch_assoc()): ?>
                    <option value="<?php echo $part['part_id']; ?>" 
                        ...
                        <?php echo $part['name']; ?> (RWF <?php echo number_format($part['unit_price'], 2); ?> per unit, Qty: <?php echo $part['quantity']; ?>)
                    </option>
                <?php endwhile; ?>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="quantity-info" id="quantity_info"></div>
        </div>
        
        <div class="form-group">
            <label for="stock_out_date">Date:</label>
            <input type="date" id="stock_out_date" name="stock_out_date" 
                   value="<?php echo isset($_POST['stock_out_date']) ? $_POST['stock_out_date'] : date('Y-m-d'); ?>" required>
        </div>
        ...
        <div class="form-buttons">
            ...
            <a href="view_stock_out.php" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
</div>
